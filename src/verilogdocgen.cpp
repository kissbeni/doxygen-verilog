...
QCString VerilogDocGen::convertTypeToString(int type,bool sing)
{
 ...
 // if(ml==NULL) return;
  ...
  VerilogDocGen::writePlainVerilogDeclarations(ml,ol,cd,nd,fd,gd,type);
    ...
    while ((mg=mgli.current()))
    {
     // assert(0);
      ...
      //printf("mg->header=%s\n",mg->header().data());
        ...
        VerilogDocGen::writePlainVerilogDeclarations(mg->members(),ol,cd,nd,fd,gd,type);
...
void VerilogDocGen::writePlainVerilogDeclarations(
...
void VerilogDocGen::writeVerilogDeclarations(MemberDef* mdef,OutputList &ol,
  ...
  /*if (!Config_getString("GENERATE_TAGFILE").isEmpty()) // TODO: move to VerilogDocGen::writeTagFile
  {
...
MemberDef* VerilogDocGen::findMember(QCString& className, QCString& memName,int type)
{
  ...
  //  if(!cd) return NULL;
   ...
   mdef=VerilogDocGen::findMemberDef(cd,memName,MemberListType_variableMembers);
     if(mdef) return mdef;

 //    mdef=VerilogDocGen::findMemberDef(cd,memName,MemberList:: pubMethods);
 //   if(mdef) return mdef;
 ...
 MemberDef* VerilogDocGen::findMemberDef(ClassDef* cd,const QCString& key,MemberListType type)
 {
  ...
  if(ml==NULL) return NULL;
    MemberListIterator fmni(*ml);
      ...
      ClassDef* cdef= VhdlDocGen::getClass(md->name());
       if(cdef){
          MemberDef* mdd=VerilogDocGen::findMemberDef(cdef,memName,MemberListType_variableMembers);
